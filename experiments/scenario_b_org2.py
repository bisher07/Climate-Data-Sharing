...
def main() -> None:
    ...
    ingestion = IngestionProvenanceAgent("org2.ingestion", org2_ledger, clock=clock)
    ...
    negotiation = AccessNegotiationAgent("org2.negotiation", org2_ledger, clock=clock)
    pipeline = Org2ValidationPipeline(ingestion, quality, policy)
    ...
    print("  raw reading      {'air_temperature': 27.9degC, 'pm2_5': 18.0ug/m3}")
    ...
    print(f"  quality score    {hot.quality.quality_score:.3f}  (low, but plausible)")
    ...
    org1_ingestion.submit_anchor(
        org1_ingestion.prepare_anchor("OBS-ORG1-1200", org1_record, calibration)
    )
    ...
    print("  the flag records that they differ and by how much. It does not say Org1 is wrong,")
    print("  and it granted Org2 nothing in Org1's namespace.")

    rule("6. A reading Org1 and Org2 agree on")
    ...
    rule("7. Org2 asks Org1 for data")
    ...
    print(f"  granted so far   {negotiation.granted_assets()}")
    ...
    print(f"  Org1 answers     approved")
    print(f"  outstanding      {[r.id for r in negotiation.outstanding_requests()]}")
    print(f"  Org2 may use     {negotiation.may_use(station)}  "
          f"— because Org1 said so, in Org1's namespace")

    rule("8. Org2 is asked for compliance-sensitive data")
    org1_ledger.submit(Fn.CREATE_ACCESS_REQUEST, request=AccessRequestSubmission(
        ...
        requested=[AssetRef(owner_org=ORG2, doc_type=DocType.SENSOR, asset_id="SN-AQ-031")],
    ).model_dump(mode="json"))
    for request in policy.pending_access_requests():
        decision = policy.review_access_request(request)
        print(f"  {request.id}      {'APPROVED' if decision.approved else 'REFUSED'}")
        print(f"          {decision.reasons[0]}")

    rule("9. Org2 is asked to speak for Org1")
    print(f"  {policy.review_foreign_data(ORG1, 'an Org1 temperature observation').reasons[0]}")

    rule("10. Audit trail")
    trail = ingestion.audit_trail + quality.audit_trail + policy.audit_trail
    for entry in sorted(trail, key=lambda e: e.timestamp)[:14]:
        print(f"  {entry.timestamp:%H:%M:%S}  {entry.agent_id:16} {entry.action:24} {entry.outcome}")
